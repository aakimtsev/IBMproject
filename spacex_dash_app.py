...
def select(input1):
    if input1 == 'All Sites':
        new_df = spacex_df.groupby(['Launch Site'])["class"].count().reset_index()
        new_df.columns = ['Launch Site', 'count']
        fig = px.pie(new_df, values='count', names='Launch Site', title='Total Success Launches by Site')
    else:
        new_df = spacex_df[spacex_df["Launch Site"] == input1]["class"].value_counts().reset_index()
        new_df.columns = ['name', 'count']
        # Replace 0 and 1 with "not successful" and "Successful" in the labels
        labels = {0: "Not Successful", 1: "Successful"}
        new_df['name'] = new_df['name'].map(labels)
        fig = px.pie(new_df, values='count', names='name', title='Total Success Launches for ' + input1)
    return fig
